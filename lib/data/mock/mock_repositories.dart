...
final screeningHubSnapshotProvider =
    FutureProvider<ScreeningHubSnapshot>((ref) async {
  ref.watch(mockAppStoreProvider);
...
class MockCaseRepository implements CaseRepository {
  ...
  Future<List<CaseSummary>> getCaseSummaries() async {
    final state = ref.read(mockAppStoreProvider);
    return state.cases.map((bundle) => bundle.detail.summary).toList(growable: false);
  }
  ...
  Future<CaseSummary> createCase({
    ...
    final bundle = ref.read(mockAppStoreProvider.notifier).createCase(
...
class MockDashboardRepository implements DashboardRepository {
  ...
  Future<DashboardSnapshot> getSnapshot() async {
    final state = ref.read(mockAppStoreProvider);
    final summaries = state.cases.map((bundle) => bundle.detail.summary).toList(growable: false);
    ...
    return DashboardSnapshot(
      metrics: <DashboardMetric>[
        DashboardMetric(
          ...
          helper: '补录与冲突核对',
          deltaLabel: '${openTasks.where((task) => task.isBlocking).length} 个阻断项',
          isPositive: false,
        ),
        DashboardMetric(
          id: 'ready',
...
class MockUploadRepository implements UploadRepository {
  ...
  Future<UploadJob> createUpload({
    ...
    return ref.read(mockAppStoreProvider.notifier).createUpload(
          patientId: patientId,
...
class MockScreeningRepository implements ScreeningRepository {
  ...
  @override
  Future<void> supplementFields(String caseId, Map<String, String> values) async {
...
class MockSearchRepository implements SearchRepository {
  ...
  Future<List<SearchResult>> searchCases(SearchFilter filter) async {
    final state = ref.read(mockAppStoreProvider);
    final bundles = state.cases.where((bundle) {
      ...
      final matchesPrimarySite =
          filter.primarySite == null || summary.primarySite == filter.primarySite;
      final matchesTumorType =
          filter.tumorType == null || summary.tumorType == filter.tumorType;
      final matchesStage = filter.stage == null || summary.stage == filter.stage;
      final matchesLine = filter.lineOfTherapy == null ||
      ...
      final matchesLiverRisk =
          filter.hasLiverRisk == null || summary.hasLiverRisk == filter.hasLiverRisk;
      final matchesEcog =
          filter.ecogMax == null || summary.ecog <= filter.ecogMax!;
      final matchesComorbidity = filter.comorbidity == null ||
          detail.comorbidities.contains(filter.comorbidity);
      final matchesScreening = filter.screeningStatusLabel == null ||
    ...
    return bundles.map((bundle) {
      ...
      final reasons = <String>[
        ...
        if (filter.drugClass != null) '药物类别匹配 ${_drugClassOf(summary.currentRegimen)}',
...
class MockTaskRepository implements TaskRepository {
  ...
  Future<void> resolveTask(String taskId, {String? resolvedValue}) async {
    ref.read(mockAppStoreProvider.notifier).resolveTask(taskId, resolvedValue: resolvedValue);
  }
}
...
class MockScreeningHubRepository {
  ...
  Future<ScreeningHubSnapshot> getHubSnapshot() async {
    ...
    final readyCount = bundles
        .where((b) =>
            b.detail.summary.screeningStatus == ScreeningStatus.ready)
        .length;
    final partialCount = bundles
        .where((b) =>
            b.detail.summary.screeningStatus == ScreeningStatus.partial)
        .length;
    final notReadyCount = bundles
        .where((b) =>
            b.detail.summary.screeningStatus == ScreeningStatus.notReady)
        .length;
    final totalCases = bundles.length;
    final overallProgress =
        totalCases == 0 ? 0.0 : readyCount / totalCases;

    final projects = bundles.map((bundle) {
      final summary = bundle.detail.summary;
    ...
    final topBlockingFields = blockingMap.entries
        .map((entry) => BlockingFieldSummary(
              fieldName: entry.key,

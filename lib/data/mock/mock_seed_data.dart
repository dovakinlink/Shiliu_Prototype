...
const _icd10Codes = <String>[
  'C34.9', 'C16.9', 'C15.5', 'C50.9', 'C56', 'C18.9', 'C22.0', 'C37',
];
...
const _stageSystems = <String>[
  'AJCC 8th', 'AJCC 8th', 'AJCC 8th', 'AJCC 8th',
  'FIGO 2014', 'AJCC 8th', 'BCLC+AJCC 8th', 'Masaoka-Koga',
];

const _tnmTs = <String>['T2b', 'T3', 'T3', 'T2', 'T3c', 'T3', 'T2', 'T3'];
const _tnmNs = <String>['N2', 'N2', 'N1', 'N1', 'N1', 'N1', 'N0', 'N1'];
const _tnmMs = <String>['M1a', 'M0', 'M1', 'M0', 'M1', 'M1', 'M0', 'M1'];
const _ajccStages = <String>[
  'IVA', 'IIIB', 'IVB', 'IIIB', 'IVA', 'IVB', 'IIIB', 'IVA',
];

// Molecular results vary by tumor type (slot index)
List<List<MolecularResult>> _buildMolecularResultSets() => <List<MolecularResult>>[
  // 0: 肺
  const <MolecularResult>[
    MolecularResult(gene: 'EGFR', variant: 'L858R', status: MolecularStatus.positive, sampleType: '组织'),
    MolecularResult(gene: 'ALK', variant: '-', status: MolecularStatus.negative, sampleType: '组织'),
    MolecularResult(gene: 'KRAS', variant: '-', status: MolecularStatus.negative, sampleType: '组织'),
    MolecularResult(gene: 'ROS1', variant: '-', status: MolecularStatus.negative, sampleType: '组织'),
  ],
  // 1: 胃
  const <MolecularResult>[
    MolecularResult(gene: 'HER2', variant: 'IHC 2+/FISH-', status: MolecularStatus.negative, sampleType: '组织'),
    MolecularResult(gene: 'KRAS', variant: '-', status: MolecularStatus.notTested, sampleType: '-'),
  ],
  // 2: 食管
  const <MolecularResult>[
    MolecularResult(gene: 'EGFR', variant: '-', status: MolecularStatus.notTested, sampleType: '-'),
  ],
  // 3: 乳腺
  const <MolecularResult>[
    MolecularResult(gene: 'HER2', variant: 'IHC 1+', status: MolecularStatus.positive, sampleType: '组织'),
    MolecularResult(gene: 'BRCA1/2', variant: '-', status: MolecularStatus.negative, sampleType: '血液'),
  ],
  // 4: 卵巢
  const <MolecularResult>[
    MolecularResult(gene: 'BRCA1', variant: '5382insC', status: MolecularStatus.positive, sampleType: '血液'),
    MolecularResult(gene: 'HRD', variant: 'Score 56', status: MolecularStatus.positive, sampleType: '组织'),
  ],
  // 5: 结直肠
  const <MolecularResult>[
    MolecularResult(gene: 'KRAS', variant: 'G12D', status: MolecularStatus.positive, sampleType: '组织'),
    MolecularResult(gene: 'BRAF', variant: '-', status: MolecularStatus.negative, sampleType: '组织'),
    MolecularResult(gene: 'NRAS', variant: '-', status: MolecularStatus.negative, sampleType: '组织'),
  ],
  // 6: 肝
  const <MolecularResult>[
    MolecularResult(gene: 'TERT', variant: 'promoter mut', status: MolecularStatus.positive, sampleType: '组织'),
  ],
  // 7: 纵隔
  const <MolecularResult>[
    MolecularResult(gene: 'KIT', variant: '-', status: MolecularStatus.negative, sampleType: '组织'),
  ],
];
...
const _biomarkerSets = <List<BiomarkerResult>>[
  ...
  // 2: 食管
  <BiomarkerResult>[
    BiomarkerResult(name: 'PD-L1 (CPS)', value: '15'),
  ],
  // 3: 乳腺
  <BiomarkerResult>[
  ...
  // 4: 卵巢
  <BiomarkerResult>[
    BiomarkerResult(name: 'BRCA1', value: '致病突变'),
    BiomarkerResult(name: 'CA-125', value: '340 U/mL'),
  ],
  // 5: 结直肠
  <BiomarkerResult>[
    BiomarkerResult(name: 'MSI', value: 'MSS'),
...
List<TreatmentLine> _treatmentLinesForSlot(int slot, int maxLine, DateTime now) {
  final lines = <TreatmentLine>[];
  final regimenData = <List<_RegimenSpec>>[
    // 0: 肺
    [_RegimenSpec('培美曲塞+顺铂+帕博利珠单抗', '化免联合', ['培美曲塞', '顺铂', '帕博利珠单抗'], ['化疗-叶酸拮抗', '化疗-铂类', 'PD-1']),
     _RegimenSpec('帕博利珠单抗单药维持', '免疫单药', ['帕博利珠单抗'], ['PD-1'])],
    // 1: 胃
    [_RegimenSpec('SOX + 信迪利单抗', '化免联合', ['奥沙利铂', '替吉奥', '信迪利单抗'], ['化疗-铂类', '化疗-氟尿嘧啶', 'PD-1'])],
    // 2: 食管
    [_RegimenSpec('紫杉醇+纳武利尤单抗', '化免联合', ['紫杉醇', '纳武利尤单抗'], ['化疗-紫杉', 'PD-1'])],
    // 3: 乳腺
    [_RegimenSpec('T-DXd (ADC)', 'ADC', ['T-DXd'], ['ADC'])],
    // 4: 卵巢
    [_RegimenSpec('紫杉醇脂质体+贝伐珠单抗', '化疗+抗血管', ['紫杉醇脂质体', '贝伐珠单抗'], ['化疗-紫杉', '抗VEGF'])],
    // 5: 结直肠
    [_RegimenSpec('FOLFIRI + 贝伐珠单抗', '化疗+抗血管', ['伊立替康', '氟尿嘧啶', '亚叶酸钙', '贝伐珠单抗'], ['化疗-拓扑异构酶', '化疗-氟尿嘧啶', '辅助', '抗VEGF'])],
    // 6: 肝
    [_RegimenSpec('阿替利珠单抗+贝伐珠单抗', '免疫+抗血管', ['阿替利珠单抗', '贝伐珠单抗'], ['PD-L1', '抗VEGF'])],
    // 7: 纵隔
  ...
  for (int l = 1; l <= maxLine && l <= specs.length; l++) {
    ...
    final endDay = l < maxLine ? now.subtract(Duration(days: 200 - l * 90)) : null;
    lines.add(TreatmentLine(
      lineNo: l,
      regimenName: s.name,
      regimenType: s.type,
      startDate: conciseDate(startDay),
      endDate: endDay != null ? conciseDate(endDay) : null,
      drugs: List<DrugExposure>.generate(s.drugs.length, (di) => DrugExposure(
...
LabPanel _labPanel(DateTime date, bool liverRisk) {
  return LabPanel(
    ...
    results: <LabResult>[
      LabResult(testName: 'ALT', value: liverRisk ? 128 : 25, unit: 'U/L', refLow: 7, refHigh: 40, isAbnormal: liverRisk, ctcaeGrade: liverRisk ? 2 : null),
      LabResult(testName: 'AST', value: liverRisk ? 96 : 22, unit: 'U/L', refLow: 13, refHigh: 35, isAbnormal: liverRisk, ctcaeGrade: liverRisk ? 2 : null),
      LabResult(testName: 'TBil', value: liverRisk ? 32.1 : 12.4, unit: 'µmol/L', refLow: 3.4, refHigh: 20.5, isAbnormal: liverRisk, ctcaeGrade: liverRisk ? 1 : null),
      LabResult(testName: 'Alb', value: liverRisk ? 31.2 : 42.5, unit: 'g/L', refLow: 40, refHigh: 55, isAbnormal: liverRisk),
      LabResult(testName: 'Cr', value: 68, unit: 'µmol/L', refLow: 41, refHigh: 81, isAbnormal: false),
      LabResult(testName: 'eGFR', value: 95, unit: 'mL/min/1.73m²', refLow: 90, refHigh: 120, isAbnormal: false),
      LabResult(testName: 'WBC', value: 5.8, unit: '×10⁹/L', refLow: 3.5, refHigh: 9.5, isAbnormal: false),
      LabResult(testName: 'ANC', value: 3.2, unit: '×10⁹/L', refLow: 1.8, refHigh: 6.3, isAbnormal: false),
      LabResult(testName: 'HGB', value: 118, unit: 'g/L', refLow: 115, refHigh: 150, isAbnormal: false),
      LabResult(testName: 'PLT', value: 185, unit: '×10⁹/L', refLow: 125, refHigh: 350, isAbnormal: false),
    ],
  );
}
...
List<ImagingRecord> _imagingForSlot(int slot, DateTime now) {
  ...
  return <ImagingRecord>[
    ImagingRecord(
      ...
      response: const <String>['PR', 'PR', 'SD', 'CR', 'PR', 'SD', 'PR', 'PD'][slot],
    ),
    ImagingRecord(
...
// Adverse events
List<AdverseEventRecord> _adverseEventsForCase(int index, int slot, bool liverRisk, DateTime now) {
  final events = <AdverseEventRecord>[];
  if (liverRisk) {
    events.add(AdverseEventRecord(
      aeTerm: '免疫相关性肝损伤',
      grade: 2,
...
List<MockCaseBundle> buildSeedCaseBundles() {
  const patientNames = <String>[
    '吴女士', '赵先生', '周女士', '钱先生', '孙女士', '李先生',
    '郑女士', '王先生', '冯女士', '陈先生', '褚女士', '卫先生',
    '蒋女士', '沈先生', '韩女士', '杨先生', '朱女士', '秦先生',
    '尤女士', '许先生', '何女士', '吕先生', '施女士', '张先生',
  ];

  const primarySites = <String>['肺', '胃', '食管', '乳腺', '卵巢', '结直肠', '肝', '纵隔'];
  const tumorTypes = <String>[
    '非小细胞肺癌', '胃癌', '食管鳞癌', 'HER2 低表达乳腺癌',
    '高级别浆液性卵巢癌', '结直肠腺癌', '肝细胞癌', '胸部罕见肿瘤',
  ];
  const histologies = <String>[
    '肺腺癌', '胃腺癌', '鳞状细胞癌', '浸润性导管癌',
    '浆液性癌', '中分化腺癌', '肝细胞癌', '胸腺鳞癌',
  ];
  const diseaseGroups = <String>[
    '胸部肿瘤', '消化道肿瘤', '消化道肿瘤', '乳腺肿瘤',
    '妇科肿瘤', '消化道肿瘤', '肝胆肿瘤', '罕见肿瘤',
  ];
  const stages = <String>['IV期', 'III期', 'IV期', 'IIIB期', 'IV期', 'IV期', 'IIIB期', 'IV期'];
  const regimens = <String>[
    'PD-1 单药维持', 'SOX + PD-1', '紫杉醇 + 纳武利尤单抗', 'ADC 序贯治疗',
    '紫杉醇脂质体 + 贝伐珠单抗', 'FOLFIRI + 贝伐珠单抗', '阿替利珠单抗 + 抗 VEGF', '临床试验筛查中',
  ];
  const diseaseStatuses = <String>[
    '复发转移', '治疗中', '进展后', '维持治疗', '复发转移', '治疗中', '随访中', '筛查评估',
  ];
  ...
  const biomarkerLabels = <String>[
    'PD-L1 TPS 70%', 'HER2 低表达', 'CPS 10', 'ER 40%',
    'BRCA 未检', 'KRAS G12D', 'HBV DNA 监测中', 'KIT 未见异常',
  ];

  final now = DateTime(2026, 4, 14);
  final molecularSets = _buildMolecularResultSets();

  return List<MockCaseBundle>.generate(4, (index) {
    ...
    final ecog = index % 4 == 0 ? 2 : index.isEven ? 1 : 0;
    ...
    final tags = <String>[
      diseaseGroups[slot],
      if (baseStatus != ScreeningStatus.notReady && regimens[slot].contains('PD-1') && !regimens[slot].contains('+')) 'PD-1单药',
      if (liverRisk) '肝功异常',
    ...
    final summary = CaseSummary(
      ...
      stage: stages[slot],
      lineOfTherapy: line,
      currentRegimen: baseStatus == ScreeningStatus.notReady ? '待补录' : regimens[slot],
    ...
    final structuredFields = <StructuredField>[
      StructuredField(
        id: histologyFieldId, label: '病理类型', value: histologies[slot],
        group: '诊断', confidence: FieldConfidence.verified,
        eventId: diagnosisEventId, evidenceId: pathologyEvidenceId, isRequired: true,
      ),
      StructuredField(
        id: stageFieldId, label: '临床分期', value: stages[slot],
        group: '分期',
        confidence: hasConflict ? FieldConfidence.aiMedium : FieldConfidence.aiHigh,
        eventId: imagingEventId, evidenceId: imagingEvidenceId, isRequired: true,
      ),
      StructuredField(
        id: regimenFieldId, label: '当前方案',
        value: blockingFields.contains('当前方案') ? '待补录' : regimens[slot],
        group: '治疗',
        confidence: blockingFields.contains('当前方案') ? FieldConfidence.missing : FieldConfidence.aiHigh,
        eventId: treatmentEventId, evidenceId: orderEvidenceId, isRequired: true, isBlocking: true,
      ),
      StructuredField(
        id: ecogFieldId, label: 'ECOG评分',
        value: blockingFields.contains('ECOG评分') ? '待补录' : '$ecog',
        group: '筛查',
        confidence: hasConflict
            ? FieldConfidence.conflict
            : blockingFields.contains('ECOG评分') ? FieldConfidence.missing : FieldConfidence.aiMedium,
        eventId: labEventId, evidenceId: labEvidenceId, isRequired: true, isBlocking: true,
      ),
      StructuredField(
        id: liverFieldId, label: '肝功状态',
        value: liverRisk ? 'ALT Grade 2' : '近期稳定',
        group: '安全性',
        confidence: liverRisk ? FieldConfidence.aiMedium : FieldConfidence.aiHigh,
        eventId: labEventId, evidenceId: labEvidenceId,
      ),
      StructuredField(
        id: biomarkerFieldId, label: '关键分子标志物',
        value: reminderFields.contains('关键分子标志物') ? '待补录' : biomarkerLabels[slot],
        group: '标志物',
        confidence: reminderFields.contains('关键分子标志物') ? FieldConfidence.missing : FieldConfidence.aiHigh,
        eventId: diagnosisEventId, evidenceId: pathologyEvidenceId,
      ),
    ];
    ...
    final timeline = <TimelineEvent>[
      TimelineEvent(
        id: diagnosisEventId, title: '首次确诊', subtitle: summary.tumorType,
        ...
        evidenceIds: <String>[pathologyEvidenceId], isImportant: true,
      ),
      TimelineEvent(
        id: imagingEventId, title: '基线影像', subtitle: '${summary.primarySite}病灶评估',
      ...
      TimelineEvent(
        id: treatmentEventId, title: '当前治疗方案', subtitle: '${summary.lineOfTherapy} 线治疗',
        ...
        evidenceIds: <String>[orderEvidenceId], isImportant: true,
      ),
      TimelineEvent(
        id: labEventId, title: '近期安全性评估',
    ...
    final evidenceDocuments = <EvidenceDocument>[
      EvidenceDocument(
        id: pathologyEvidenceId, title: '病理报告', modality: DocumentModality.pdf,
        source: '外院上传',
        dateLabel: conciseDate(now.subtract(Duration(days: 280 + index * 3))),
        summary: '病理诊断明确，含关键免疫组化与标志物描述。',
        anchor: EvidenceAnchor(label: '病理诊断段', locator: '第 1 页 · 框 02',
      ...
      EvidenceDocument(
        id: imagingEvidenceId, title: '基线影像报告', modality: DocumentModality.pdf,
        source: '影像中心',
        dateLabel: conciseDate(now.subtract(Duration(days: 240 + index * 2))),
        summary: '完成胸腹盆增强评估，形成 RECIST 基线。',
        anchor: EvidenceAnchor(label: '影像结论', locator: '第 2 页 · 框 01',
          excerpt: '考虑 ${summary.stage}，病灶大小较前增大。'),
        linkedFieldIds: <String>[stageFieldId],
        eventId: imagingEventId,
      ),
      EvidenceDocument(
        id: orderEvidenceId, title: '治疗医嘱与病程摘要', modality: DocumentModality.image,
        source: '病区拍照',
        dateLabel: conciseDate(now.subtract(Duration(days: 120 - index))),
        summary: '抽取当前治疗方案、线数及最近一次给药日期。',
        anchor: EvidenceAnchor(label: '医嘱截图', locator: '图片 1 · 框 05',
          excerpt: '当前方案：${regimens[slot]}。'),
        linkedFieldIds: <String>[regimenFieldId],
        eventId: treatmentEventId,
      ),
      EvidenceDocument(
        id: labEvidenceId, title: '近期化验与随访', modality: DocumentModality.text,
        source: '结构化录入',
        dateLabel: conciseDate(now.subtract(Duration(days: (index % 7) + 1))),
        summary: '包含 ECOG、肝功能及筛查回填记录。',
        anchor: EvidenceAnchor(label: '实验室摘要', locator: '时间点 T${(index % 4) + 1}',
    ...
    final detail = CaseDetail(
      ...
      infectionStatus: hasHbv
          ? InfectionInfo(hbvStatus: slot == 6 ? 'HBsAg 阳性，抗病毒治疗中' : 'HBsAg 阴性/HBcAb 阳性')
          : null,
    );
